...
- **0.2.2**: Claude が作業中または思考中のとき、ハードウェアランプが黄色で点滅します。完了すると緑に戻り、より小さなシリアルデータで LED 更新が安定します。
- **0.2.0**: ESP32-C3 Mini 1 ハードウェアランプ対応を追加しました。`device` コマンドと `watch --device` が使えます。
- **0.1.1**: インストール可能な CLI、Claude Code hooks、ターミナル状態表示、デスクトップ通知、設定 UI、診断コマンドを追加しました。
...
`doctor` は `.agent-pulse`、CLI コマンド、`.claude/settings.json`、statusLine、hooks を確認します。

状態が変わらない場合:

1. 監視したいプロジェクトディレクトリにいることを確認します。
2. `agent-traffic-light-monitor init` を実行します。
3. Claude Code セッションを再起動して `.claude/settings.json` を再読み込みします。
