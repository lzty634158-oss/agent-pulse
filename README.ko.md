...
- **0.2.2**: Claude가 작업하거나 생각하는 동안 하드웨어 램프가 노란색으로 깜빡이고, 완료되면 초록색으로 바뀝니다. 더 작은 시리얼 데이터만 보내 LED 업데이트도 더 안정적입니다.
- **0.2.0**: ESP32-C3 Mini 1 하드웨어 램프 지원을 추가했습니다. `device` 명령과 `watch --device`를 사용할 수 있습니다.
- **0.1.1**: 설치 가능한 CLI, Claude Code hooks, 터미널 상태 보기, 데스크톱 알림, 설정 UI, 진단 명령을 추가했습니다.
